...
{
public:
	NetworkInitTask() : Thread(configMAX_PRIORITIES - 1, 2048, "network_init") {}
	...
	run(void *parameter)
	{
		...
		if (buffer)
		{
			...
			while (true)
			{
				...
				if (bytes <= 0) break;
				if (FreeRTOS_send(connectedSocket, httpText, sizeof(httpText) - 1, 0) < 0) break;
			}
		}
		...
		do
		{
			if (FreeRTOS_recv(connectedSocket, buffer, ipconfigTCP_MSS, 0) < 0) break;
...
main()
{
	...
	Ethernet::Port::connect<
		Ethernet::RMII_Ref_Clk::Refclk, Ethernet::RMII_Mdc::Mdc, Ethernet::RMII_Mdio::Mdio,
		Ethernet::RMII_Crs_Dv::Rcccrsdv, Ethernet::RMII_Tx_En::Txen, Ethernet::RMII_Tx_D0::Txd0,
		Ethernet::RMII_Tx_D1::Txd1, Ethernet::RMII_Rx_D0::Rxd0, Ethernet::RMII_Rx_D1::Rxd1>();
...
vApplicationIPNetworkEventHook(eIPCallbackEvent_t eNetworkEvent)
{
	...
	if (eNetworkEvent != eNetworkUp) return;

	if (not taskCreated)
	{
		xTaskCreate(HttpServerListener::run, HttpServerListener::name, configMINIMAL_STACK_SIZE * 2,
					0, configMAX_PRIORITIES - 2, 0);
		taskCreated = true;
	}
