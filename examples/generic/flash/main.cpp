...
main()
{
	...
	{
		const Flash::SectorType nsector = Flash::getSector(offset);
		if (sector != nsector) {
	...
	{
		uint32_t err{0};
		const Flash::SectorType sector_start = Flash::getSector(Flash::Size/2);
		const Flash::SectorType sector_end = Flash::getSector(Flash::Size);
...

	{
		...
		{
			Flash::PageType page = Flash::getPage(dst_addr);
			for (;
					 Flash::getPage(dst_addr) == page;
