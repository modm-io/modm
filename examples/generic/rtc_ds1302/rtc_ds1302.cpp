...
struct ds1302_config : public xpcc::ds1302::Config
{
	using Ce   = Board::D8;
	using Io   = Board::D9;
	using Sclk = Board::D10;
};
using ds1302 = xpcc::Ds1302< ds1302_config >;
...
main()
{
	...
	XPCC_LOG_DEBUG << xpcc::endl << "Welcome to RTC DS1302 demo." << xpcc::endl;
	...
	xpcc::PeriodicTimer blinkTimer(250);
	...
	while(true)
	{
		if (blinkTimer.execute()) {
			XPCC_LOG_DEBUG.printf("\b%c", cc[cycle++]);
			cycle %= 4;
			Board::Leds::toggle();
		}
		if (timeout.execute())
		{
			...
			XPCC_LOG_DEBUG.printf("\b* %2d.%03d seconds from CPU are %2d seconds from RTC.\n",
				tt / 1000,
				tt % 1000,
				seconds);
