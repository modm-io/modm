...
:	data(data), i2cTask(I2cTask::Idle), i2cSuccess(0),
	...
	i2cBuffer{0,0,0,0}, logicBuffer{0,0}
{
}
...
xpcc::Vl6180<I2cMaster>::initialize()
{
	...
	logicBuffer[1] = (i2cBuffer[2]) ? 0 : 29;

	for (; logicBuffer[1] < 40; logicBuffer[1]++)
	{
		i2cBuffer[2] = configuration[logicBuffer[1]].value;
		logicBuffer[0] &= CO_CALL(write(Register(configuration[logicBuffer[1]].reg), i2cBuffer[2]));

		if (!logicBuffer[0]) CO_RETURN(false);
	}

	if (logicBuffer[0])
		CO_CALL( write(Register::SYSTEM__FRESH_OUT_OF_RESET, 0) );

	CO_END_RETURN(logicBuffer[0]);
}
...
xpcc::Vl6180<I2cMaster>::readSensor(bool isDistance)
{
	...
	if ( CO_CALL(write(isDistance ? Register::SYSRANGE__START : Register::SYSALS__START, uint8_t(Start::StartStop))) )
	{
		...
		if ( CO_CALL(read(isDistance ? Register::RESULT__RANGE_VAL : Register::RESULT__ALS_VAL, i2cBuffer+2, isDistance ? 1 : 2)) )
		{
			if (isDistance)
				data.data[0] = i2cBuffer[2];
			else {
				data.data[0] = i2cBuffer[2];
				data.data[1] = i2cBuffer[3];
			}
			...
			if ( CO_CALL(write(Register::SYSTEM__INTERRUPT_CLEAR, (InterruptClear::Range | InterruptClear::ALS | InterruptClear::Error).value)) )
			{
				...
				{
					// read the byte
					if ( !CO_CALL(read(isDistance ? Register::RESULT__RANGE_STATUS : Register::RESULT__ALS_STATUS, i2cBuffer[2])) )
						CO_RETURN(false);
