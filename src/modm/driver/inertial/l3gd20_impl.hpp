...
modm::L3gd20<Transport>::configure(Scale scale, MeasurementRate rate)
{
	...
	if (this->write(i(Register::CTRL_REG1), rawBuffer[0]))
	{
		return this->write(i(Register::CTRL_REG4), rawBuffer[3]);
	}
	return false;
}
...
{
	if (this->read(i(Register::OUT_TEMP) | Transport::AddressIncrement, rawBuffer + 6, 12))
	{
		// copy the memory
...
{
	if (this->read(reg, rawBuffer[6]))
	{
		rawBuffer[6] = (rawBuffer[6] & ~clearMask) | setMask;
		return this->write(reg, rawBuffer[6]);
	}
