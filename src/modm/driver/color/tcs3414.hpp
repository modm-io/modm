...
struct tcs3414
{
	...
	enum class IntegrationMode : uint8_t
	{
		...
		ADC_CTR = 0b010000,   //!< use the ADC enable bit field in the control register to start and
							  //!< stop integration
...
public:
	...
	static inline bool
	configure(const Gain gain = Gain::DEFAULT, const Prescaler prescaler = Prescaler::DEFAULT,
	...
	setGain(const Gain gain = Gain::DEFAULT, const Prescaler prescaler = Prescaler::DEFAULT)
	{
	...
	modm::ResumableResult<bool>
	setIntegrationTime(const IntegrationMode mode = IntegrationMode::DEFAULT,
					   const NominalIntegrationTime time = NominalIntegrationTime::DEFAULT)
	{
	...
	initialize()
	{
		return writeRegister(RegisterAddress::CONTROL,
							 0b11);  // control to power up and start conversion
	};

	modm::ResumableResult<bool>
	configure(const Gain gain = Gain::DEFAULT, const Prescaler prescaler = Prescaler::DEFAULT,
			  const IntegrationMode mode = IntegrationMode::DEFAULT, const uint8_t time = 0);
...
private:
	...
	{
		return writeRegister(RegisterAddress::TIMING, static_cast<uint8_t>(mode) | time);
	}
...
private:
	...
	modm::ResumableResult<bool>
	readRegisters(const RegisterAddress address, uint8_t* const values, const uint8_t count = 1);

	modm::ResumableResult<bool>
	writeRegister(const RegisterAddress address, const uint8_t value);
