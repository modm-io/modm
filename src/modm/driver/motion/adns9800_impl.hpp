...
{
	Cs::reset();
	modm::this_fiber::sleep_for(100us); // tSRAD
...
modm::Adns9800< Spi, Cs>::readReg(Register const reg)
{
	...
	modm::delay_ns(120); // tSCLK-NCS for read operation is 120ns
	Cs::set();
	modm::this_fiber::sleep_for(19us); // tSRW/tSRR (=20us) minus tSCLK-NCS
...
modm::Adns9800< Spi, Cs>::writeReg(Register const reg, uint8_t const data)
{
	...
	modm::this_fiber::sleep_for(20us); // tSCLK-NCS for write operation
	Cs::set();
	modm::this_fiber::sleep_for(100us); // tSWW/tSWR (=120us) minus tSCLK-NCS. Could be shortened, but is looks like a safe lower bound
}
...
modm::Adns9800< Spi, Cs >::uploadFirmware()
{
	...
	modm::this_fiber::sleep_for(10ms); // assume that the frame rate is as low as 100fps... even if it should never be that low
...
modm::Adns9800< Spi, Cs >::initialise()
{
	...
	modm::this_fiber::sleep_for(50ms); // wait for it to reboot
