...
/// @ingroup modm_driver_ili9341_spi
struct ili9341
{
protected:
	/// @cond
...
class Ili9341 : public ili9341, public modm::GraphicDisplay,
...
public:
	...
	void
	setScrollArea(uint16_t topFixedRows, uint16_t bottomFixedRows,
			uint16_t firstRow);
	void
...
private:
	...
	writeCommand(Command command) {
		...
		(void) SPI::transferBlocking(i(command));
	...
	writeCommand(Command command, uint8_t const *args, std::size_t length) {
		...
		Dc::set(); // reset to data
		if (length != 0) {
			(void) SPI::transferBlocking(const_cast<unsigned char *>(args),
					nullptr, length);
		}
	}
	void
	writeData(uint8_t const *data, std::size_t length) {
		(void) SPI::transferBlocking(const_cast<unsigned char *>(data),
				nullptr, length);
	}
	void
	...
	readData(Command command, uint8_t *buffer, std::size_t length)
	{
		...
		(void) SPI::transferBlocking(i(command) << 1);
