...
AmnbInterfaceTest::testSerialize()
{
	...
	Interface<100> interface(dev); // max 100B heap allocation!
	{
		...
		msg.setValid();
		TEST_ASSERT_EQUALS(interface.transmit(&msg), InterfaceStatus::Ok);
		// with byte stuffing!
	...
	SharedMedium::reset();
	{
		...
		AmnbTestMessage msg;
		TEST_ASSERT_EQUALS(interface.receiveHeader(&msg), InterfaceStatus::Ok);
	...
	SharedMedium::reset();
	{
		...
		TEST_ASSERT_EQUALS(interface.receiveData(&msg), InterfaceStatus::Ok);
		TEST_ASSERT_TRUE(msg.isDataValid());
...
AmnbInterfaceTest::testFailures()
{
	...
	Interface<35> interface(dev);
	{
...

		{
			...
			TEST_ASSERT_EQUALS(interface.transmit(&msg), InterfaceStatus::SyncWriteFailed);
			...
			TEST_ASSERT_EQUALS(interface.transmit(&msg), InterfaceStatus::HeaderWriteFailed);
...

	{
		...
		TEST_ASSERT_EQUALS(interface.receiveHeader(&msg), InterfaceStatus::SyncReadFailed);
		...
		TEST_ASSERT_EQUALS(interface.receiveHeader(&msg), InterfaceStatus::HeaderReadFailed);

		// CRC is wrong for no data
		SharedMedium::add_rx({0x7E, 0x7E, 0x00, 0x00, 0x00, 0x00});
		TEST_ASSERT_EQUALS(interface.receiveHeader(&msg), InterfaceStatus::HeaderInvalid);
		SharedMedium::reset();
		...
		TEST_ASSERT_EQUALS(interface.receiveHeader(&msg), InterfaceStatus::Ok);
		TEST_ASSERT_EQUALS(interface.receiveData(&msg), InterfaceStatus::DataReadFailed);
		...
		SharedMedium::add_rx({0x7E, 0x7E, 82, 20, 54, 0x5F, 36, 0, 143, 101,
				...
				5, 2, 3, 4,  6, 2, 3, 4,  7, 2, 3, 4,  8, 2, 3, 4,  9, 2, 3, 4});
		TEST_ASSERT_EQUALS(interface.receiveHeader(&msg), InterfaceStatus::Ok);
		TEST_ASSERT_EQUALS(interface.receiveData(&msg), InterfaceStatus::AllocationFailed);
		...
		TEST_ASSERT_EQUALS(interface.receiveHeader(&msg), InterfaceStatus::Ok);
		TEST_ASSERT_EQUALS(interface.receiveData(&msg), InterfaceStatus::DataInvalid);
	}
}
...
AmnbInterfaceTest::testInterlock()
{
	...
	TEST_ASSERT_EQUALS(interface.receiveHeader(&rx_msg), InterfaceStatus::MediumEmpty);
	TEST_ASSERT_EQUALS(interface.transmit(&tx_msg), InterfaceStatus::Ok);
	SharedMedium::reset();

	SharedMedium::add_rx({0x7E});
	TEST_ASSERT_EQUALS(interface.transmit(&tx_msg), InterfaceStatus::MediumBusy);
	TEST_ASSERT_EQUALS(interface.transmit(&tx_msg), InterfaceStatus::MediumBusy);
	...
	SharedMedium::add_rx({0x00});
	TEST_ASSERT_EQUALS(interface.receiveHeader(&rx_msg), InterfaceStatus::SyncReadFailed);
	TEST_ASSERT_EQUALS(interface.transmit(&tx_msg), InterfaceStatus::Ok);
	...
	TEST_ASSERT_EQUALS(interface.receiveHeader(&rx_msg), InterfaceStatus::Ok);
	TEST_ASSERT_EQUALS(interface.transmit(&tx_msg), InterfaceStatus::Ok);
	...
	SharedMedium::add_rx({207, 7, 4});
	TEST_ASSERT_EQUALS(interface.receiveHeader(&rx_msg), InterfaceStatus::HeaderReadFailed);
	TEST_ASSERT_EQUALS(interface.transmit(&tx_msg), InterfaceStatus::Ok);
	...
	TEST_ASSERT_EQUALS(interface.receiveHeader(&rx_msg), InterfaceStatus::HeaderInvalid);
	TEST_ASSERT_EQUALS(interface.transmit(&tx_msg), InterfaceStatus::Ok);
}
